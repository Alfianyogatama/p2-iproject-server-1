...
class Controller {
	...
	static async joinChat(req, res, next){
		...
		try{
			...
			const join = await talkJs.put(`${SECRET_APP_KEY_TALKJS}/conversations/${conversationId}/participants/${chatId}`, {
				notify : true,
				access : "ReadWrite"
			})
			if (join) {
				const result = await talkJs.get(`/${SECRET_APP_KEY_TALKJS}/conversations/${conversationId}`) 
	...
	static async createGroup(req, res, next){
		try{
			...
			const newGroup = await talkJs.put(`/${SECRET_APP_KEY_TALKJS}/conversations/${name}`, 
					{
					  participants: [`${chatId}`],
					  subject: subject,
			...
			if (newGroup) {
				const result = await talkJs.get(`/${SECRET_APP_KEY_TALKJS}/conversations/${name}`) 
				if (result) {}
	...
	static async sendMessage(req, res, next){
		try{
			...
			const {data} = await talkJs.post(`${appId}/conversations/${conversationId}/messages`, [{
				text : messages,
				sender : chatId,
				type : "UserMessage",
			}]) 
	...
	static async getMessages(req, res, next){
		try{
			const {conversationId} = req.body
			const {data} = await talkJs.get(`/${SECRET_APP_KEY_TALKJS}/conversations/${conversationId}/messages`)
